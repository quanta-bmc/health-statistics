#!/bin/bash
##
## Monitor dbus when RestartCause propertiesChanged, then set to the restart_cause sensor
##
type='signal'
org_interface='org.freedesktop.DBus.Properties'
member='PropertiesChanged'
service_name1='xyz.openbmc_project.State.Host'
obj_path1='/xyz/openbmc_project/control/host0/restart_cause'
obj_interface1='xyz.openbmc_project.Control.Host.RestartCause'
pattern='RestartCause'
service_name2='xyz.openbmc_project.HealthStatistics'
obj_path2='/xyz/openbmc_project/sensors/oem/restart_cause'
...
`dbus-monitor --system "type='$type', interface='$org_interface', member='$member', path='$obj_path1', arg0='$obj_interface1'" | awk '/'$pattern'/{getline;print $3;}' | awk '{ print "busctl set-property '$service_name2' '$obj_path2' '$obj_interface2' Value s "$0 }' | sh` | sh`
